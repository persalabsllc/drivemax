...
const labels: Record<string, string> = {
  name: 'Name', email: 'Email', phone: 'Phone', preferredContact: 'Preferred contact method',
  vehicleType: 'Vehicle type', budget: 'Budget range', vehicle: 'Vehicle of interest',
  downPayment: 'Planned down payment', reason: 'Reason for contacting us',
  position: 'Position or area of interest', message: 'Message', experience: 'Experience',
};

function Field({ id, label, name, type = 'text', placeholder, required = false, optional = false, autoComplete, describedBy, invalid = false }: {
  id: string;
  label: string;
  name: string;
  type?: string;
...
export default function LeadForm({ kind, online = false, vehicleId = '' }: { kind: LeadFormKind; online?: boolean; vehicleId?: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!email && !phone) {
      showError('Please add an email address or phone number so we can respond.', ['email', 'phone']);
      return;
    }
    if (preferredContact === 'Email' && !email) {
      showError('Please add an email address or choose phone call or text message as your preferred contact method.', ['email']);
      return;
    }
    if ((preferredContact === 'Phone call' || preferredContact === 'Text message') && !phone) {
      showError('Please add a phone number or choose email as your preferred contact method.', ['phone']);
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div className="form-grid">
        ...
        />
        <div className="field">
          <label htmlFor={`${kind}-preferredContact`}>Preferred contact method</label>
          <select id={`${kind}-preferredContact`} name="preferredContact" defaultValue="Email" aria-describedby={contactHintId}>
        ...
        {kind === 'vehicle' && (
          <>
            <div className="field">
              <label htmlFor={`${kind}-vehicleType`}>What are you shopping for?</label>
              <select id={`${kind}-vehicleType`} name="vehicleType" defaultValue="">
                <option value="" disabled>Choose a vehicle type</option><option>Sedan</option><option>SUV or crossover</option>
                <option>Truck</option><option>Minivan</option><option>Coupe or convertible</option><option>Not sure yet</option>
              </select>
            </div>
            <div className="field">
              <label htmlFor={`${kind}-budget`}>Target budget <span className="optional">(optional)</span></label>
              <select id={`${kind}-budget`} name="budget" defaultValue="">
                <option value="">Choose a range</option><option>Under $10,000</option><option>$10,000–$15,000</option>
                <option>$15,000–$20,000</option><option>$20,000 or more</option><option>Help me decide</option>
              </select>
            </div>
          </>
        )}
        ...
        {kind === 'contact' && (
          <div className="field field-full">
            ...
            <select id={`${kind}-reason`} name="reason" defaultValue="Vehicle availability">
              <option>Vehicle availability</option><option>Schedule a visit</option><option>Financing question</option>
              <option>Trade-in question</option><option>Existing customer</option><option>Something else</option>
            </select>
          </div>
        )}
        ...
        {kind === 'finance' && (
          <>
            <Field id={`${kind}-vehicle`} label="Vehicle of interest" name="vehicle" placeholder="Make, model, or body style" optional />
            <div className="field">
              <label htmlFor={`${kind}-downPayment`}>Planned down payment <span className="optional">(optional)</span></label>
              <select id={`${kind}-downPayment`} name="downPayment" defaultValue="">
                <option value="">Choose a range</option><option>Under $1,000</option><option>$1,000–$2,499</option>
                <option>$2,500–$4,999</option><option>$5,000 or more</option><option>Not sure yet</option>
              </select>
            </div>
          </>
        )}

        {kind === 'employment' && <Field id={`${kind}-position`} label="Position or area of interest" name="position" placeholder="Sales, service, office, etc." required />}

        <div className="field field-full">
          <label htmlFor={`${kind}-${messageName}`}>
            {kind === 'employment' ? 'Tell us about your experience' : kind === 'contact' ? 'Message' : 'Anything else we should know?'}
            {messageOptional && <span className="optional"> (optional)</span>}
          </label>
          <textarea
            id={`${kind}-${messageName}`}
            name={messageName}
            placeholder={kind === 'vehicle' ? 'Preferred makes, must-have features, mileage limits, or timing' : kind === 'finance' ? 'Vehicle needs, timing, or questions for the dealership' : 'How can Drive Max help?'}
